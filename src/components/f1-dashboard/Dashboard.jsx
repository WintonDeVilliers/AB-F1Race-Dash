...
import { useExcelMonitor } from '../services/excelMonitor';
...
export default function Dashboard() {
  ...
  // Use Excel monitor hook to get real-time data
  const { data: excelData, loading: excelLoading, error: excelError } = useExcelMonitor();
  ...
  // Prioritize Excel monitor data over local data
  const dashboardData = excelData || localData;
  
  // Debug: Log what data we're receiving
  console.log('Dashboard received excelData:', excelData);
  console.log('Dashboard received localData:', localData);
  console.log('Dashboard using dashboardData:', dashboardData);
  
  // Transform data to match component expectations  
  const displayData = dashboardData ? (() => {
    // Use Excel monitor data structure directly
    if (dashboardData.companyMetrics && dashboardData.supervisors) {
      console.log('Using Excel monitor data structure with companyMetrics:', dashboardData.companyMetrics);
      return {
        teams: dashboardData.supervisors || [],
        companyMetrics: dashboardData.companyMetrics,
        consultants: dashboardData.consultants || []
      };
    }
    
    // Fallback to old local data structure
    const consultants = dashboardData.data || [];
  ...
  const tabButtons = [
    ...
    { id: 'pitcrew', label: '🔧 Pit Crew Detail', icon: '🔧' }
  ];
        ...
        <div className={styles.mobileNavContent}>
          {tabButtons.map(tab => (
            <Button
              ...
              onClick={() => {
                console.log('=== MOBILE TAB CLICKED ===', tab.id, tab.label);
                setActiveTab(tab.id);
              }}
          ...
          <>
            {activeTab === 'total' && (
              <TotalProgressView 
                ...
                consultantData={displayData.consultants}
              />
            )}
            {activeTab === 'teams' && (
              <TeamRacingView 
            ...
            {activeTab === 'kyalami' && (
              <KyalamiCircuitView 
                data={{ supervisors: displayData.teams?.filter(team => team.circuit === 'Kyalami') }}
                companyMetrics={displayData.companyMetrics}
              />
            )}
          </>
