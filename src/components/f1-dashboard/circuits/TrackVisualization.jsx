...
export default function TrackVisualization({ teams = [], circuit = 'monaco', className = '' }) {
  ...
  useEffect(() => {
    ...
    const cars = new Map();
    teams.slice(0, 10).forEach((supervisor, index) => {
      const car = new window.PIXI.Graphics();
      const color = 0xFF6B35; // Default F1 orange
      ...
      // Distribute cars better around the track based on performance
      const initialPosition = ((supervisor.salesAchievement || 0) / 100) + (index * 0.02);
      cars.set(supervisor.id, { 
        container: car, 
        t: initialPosition % 1,
        speed: 0.0005 + ((supervisor.salesAchievement || 0) / 10000) // Variable speed based on performance
      });
    });
